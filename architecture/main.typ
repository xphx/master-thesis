...
For example, assume that we have two colors given by their RGBA values: $ & c_1 = (255, 0, 0, 255) \ & c_2 = (0, 128, 0, 255) $ We now want to determine the color that results from linearly interpolating between the two with a $t$ value of $0.3$ using the formula $t * c_1 + (1.0 - t) * c_2$.

For `f32`, we first normalize the numbers by dividing by 255, resulting in $c_1 = (1.0, 0.0, 0.0, 1.0)$ and $c_2 = (0.0, 0.5, 0.0, 1.0)$. Next, we do the interpolation: $0.3 * (1.0, 0.0, 0.0, 1.0) + 0.7 * (0.0, 0.5, 0.0, 1.0) = (0.3, 0.35, 0.0, 1.0)$. If desired, we can then scale and round the result back to `u8`, resulting in the value (77, 89, 0, 255).

For `u8`, we instead scale up the fraction and change the formula to $t * c_1 + (255 - t) * c_2$ to perform the calculations using only integers, in the end dividing the result by 255 to normalize the result back to the range of a `u8` number: $(77 * (255, 0, 0, 255) + 178 * (0, 128, 0, 255)) / 255 = ((19635, 0, 0, 19635) + (0, 22784, 0, 45390)) / 255 = (77, 89, 0, 255)$. It should be noted that we need to use `u16` numbers for intermediate results to prevent overflows.
