...
Therefore, the main thread collects multiple drawing commands and keeps track of a counter that tries to _estimate_ the cost of rendering all currently stored paths. Only once a certain threshold is reached does the main thread dispatch the whole batch of render tasks to a thread in the thread pool.
