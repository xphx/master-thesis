...
As was just shown, the vast majority of the fine rasterization pipeline consists of performing additions and multiplications between color values. We can choose to run the calculations using either 32-bit floating point numbers that are normalized between 0.0 and 1.0, or 8-bit unsigned integers ranging from 0 to 255. 
...
For `f32`, we first normalize the numbers by dividing by 255, resulting in (1.0, 0.0, 0.0, 1.0) and (0.0, 0.5, 0.0, 1.0). Next, we simply do the interpolation: $0.3 * (1.0, 0.0, 0.0, 1.0) + 0.7 * (0.0, 0.5, 0.0, 1.0) = (0.3, 0.35, 0.0, 1.0)$. If we want, we can then scale and round the result back to `u8`, resulting in the value (77, 89, 0, 255).
