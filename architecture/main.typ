...
In the beginning, the user can specify settings that should be applied while rendering. The `level` property indicates which SIMD level should be used during rendering. By default, Vello CPU will dynamically detect the highest available level on the host system, but the user can in theory override this to for example force using SSE4.2 instructions, even though the system supports AVX2. The `num_threads` property allows you to enable multi-threaded execution (see @multi-threading) or force single-threaded execution by setting the value to 0. Finally, the `render_mode` property indicates whether to prioritize speed or quality during rasterization by using `u8` or `f32` numbers (see @fine_rasterization).

The user then creates a new `RenderContext` by specifying a width and height as well as the render settings. An important property of the `RenderContext` is that it is _reusable_. This means that if the user renders a certain scene and then wants to render a second scene, they can just _reset_ the context and then reuse it instead of having to initialize a new one. This is important because during the rendering process, _Vello CPU_ will need to make a number of memory allocations which can have an impact on runtime. By reusing the same `RenderContext`, Vello CPU can _reuse_ the existing allocations in subsequent rendering operations, leading to better performance. This property is especially useful in the context of GUI rendering, where it is common to render different scenes dozens of times per second over an extended period of time.
...
Fine rasterization is where the actual "coloring" and compositing of pixels happens. This works by iterating over all commands in each wide tile and performing operations like "fill this area using a blue color" or "fill that area using a gradient color". For reason that will be explained later, all of this happen in an intermediate buffer instead of the user-supplied pixmap. Only once fine rasterization is complete do we start the process of _packing_, where the contents of our intermediate buffer are copied into the pixmap that the user provided. In the end, the fully rendered result is stored in the pixmap and can be processed further by the user.
...
+ For pixels on the edge, more work needs to be done: We need to check how the line intersects the pixel and calculate the area coverage in order to determine the correct opacity for anti-aliasing. This is clearly a much more expensive operation and we therefore want to perform this operation only for pixels where it is really necessary.

One of the core goals of strip generation is to calculate the opacity values for all pixels that could potentially be affected by anti-aliasing. However, in addition to that, we also group the pixels in a smart way and store additional "metadata" such that all the information that is needed to reproduce the original shape for rendering (which includes the opacity values for anti-aliased pixels as well as information about which areas should be completely filled and which ones should not) can be easily inferred. The key innovation is that this in our case, this happens in a very storage-efficient way by only storing anti-aliased pixels explicitly while representing filled and blank areas implicitly. 

For example, normally if you want to retain or cache the rendered version of a path, you would need to store it as an image containing all covered pixels. Assuming a resolution of 256 pixels in each dimension in RGBA format using `u8`, you would need $256 * 256 * 4  #sym.approx 262$ kilobytes to save it. If you increase this to 512 pixels, you end up with around one megabyte of data, so the storage requirements increase quadratically as the base size of the image grows.

As will be demonstrated shortly, using our method, the factor of $4$ completely falls away since we are not storing RGBA values but just single opacity values between $0$ and $255$. In addition to that, even when doubling the size of the image in both directions, the number of _anti-aliased pixels_ of a shape tends to increase linearly instead of quadratically, meaning that even very large paths can be represented in an efficient way.
...
With the sparse strip representation of the path generated, the path rendering stage of the pipeline (as shown in @overview_pipeline) concludes. The unique aspect of sparse strips is that the representation is very versatile and can be consumed in multiple ways. On the one hand, we can pass it on to the next stage to commence the actual rasterization process and render it onto the screen. 
...
=== Tile size
Initially, it was claimed that the point of strip rendering is to only calculate the opacities of _anti-aliased pixels_. However, looking at @butterfly-all-strips, it becomes apparent that this is not entirely true: There are many pixels that either have full opacity or no opacity at all and are therefore not at all anti-aliased. They were only included by virtue of being in the _vicinity_ of anti-aliased pixels and being part of the same tile.
...
$ alpha_s * C_s + alpha_b * C_b * (1 - alpha_s) $ $alpha_s$ stands for the opacity of the source color, $C_s$ for the source color, $C_b$ for the background color $alpha_b$ for the opacity of the background color. Note in particular how there are two expressions that require us to multiply the source/background color with their alpha value. This ties into the previous discussion in @premul-alpha, where it was mentioned that storing color values using premultiplied alpha can save lots of computations, and this formula should make this point more clear. If both, the source color $C_s$ (which we just computed) and background color $C_b$ (which is stored in the scratch buffer) are _already_ stored using premultiplied alpha, the formula reduces itself to $C_s + C_b * (1 - alpha_s)$. The original five arithmetic operators were reduced to just three, which leads to a very considerable performance improvement given that this calculation is performed for every pixel. Applying this formula results in a new color value which will override the existing value in the scratch buffer to eventually be reused as the new background color in future computations.
...
For example, assume that we have two colors given by their RGBA values: $ & c_1 = (255, 0, 0, 255) \ & c_2 = (0, 128, 0, 255) $ We now want to determine the color that results from linearly interpolating between the two with a $t$ of value $0.3$ using the formula $t * c_1 + (1.0 - t) * c_2$.

For `f32`, we first normalize the numbers by dividing by 255, resulting in $c_1 = (1.0, 0.0, 0.0, 1.0)$ and $c_2 = (0.0, 0.5, 0.0, 1.0)$. Next, we simply do the interpolation: $0.3 * (1.0, 0.0, 0.0, 1.0) + 0.7 * (0.0, 0.5, 0.0, 1.0) = (0.3, 0.35, 0.0, 1.0)$. If we want, we can then scale and round the result back to `u8`, resulting in the value (77, 89, 0, 255).

For `u8`, we instead scale up the fraction and change the formula to $t * c_1 + (255 - t) * c_2$ to perform the calculations using just integers, in the end dividing the result by 255 to normalize the result back to the range of a `u8`: $(77 * (255, 0, 0, 255) + 178 * (0, 128, 0, 255)) / 255 = ((19635, 0, 0, 19635) + (0, 22784, 0, 45390)) / 255 = (77, 89, 0, 255)$. Do note that we need to use `u16` numbers for intermediate results to prevent overflows.
