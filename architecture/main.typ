...
The user then creates a new `RenderContext` by specifying a width and height as well as the render settings. An important property of the `RenderContext` is that it is _reusable_. This means that if the user renders a certain scene and then wants to render a second scene, they can just _reset_ the context and then reuse it instead of having to initialize it again. This is important because during the rendering process, _Vello CPU_ will need to make a number of memory allocations which can have an impact on runtime. By reusing the same `RenderContext`, Vello CPU can reuse the existing memory allocations in subsequent rendering operations, leading to better performance. This property is especially useful in the context of GUI rendering, where it is common to render different scenes dozens of times per second over an extended period of time.
...
One of the core goals of strip generation is to calculate the opacity values for all pixels that could potentially be affected by anti-aliasing. However, in addition to that, we also group the pixels in a smart way and store additional "metadata" such that all the information that is needed to reproduce the original shape for rendering (which includes the opacity values for anti-aliased pixels as well as information about which areas should be completely filled and which ones should not) can be easily inferred. Our key innovation is to do that in a very storage-efficient way by only storing anti-aliased pixels explicitly while representing filled and blank areas implicitly. 

For example, normally to retain or cache the rendered version of a path, it would be necessary to store it as an image containing all covered pixels. Assuming a resolution of 256 pixels in each dimension in RGBA format using `u8`, this would require $256 * 256 * 4  #sym.approx 262$ kilobytes to save it. Increasing the size to 512 pixels will raise the storage requirements to around one megabyte, representing a quadratic increase in relation to the base size of the image.

As will be demonstrated shortly, using our method, the factor of $4$ (derived from the fact that we store in RGBA) completely falls away since we are not storing color values but just single opacity values between $0$ and $255$. In addition to that, even when doubling the size of the image in both directions, the number of _anti-aliased pixels_ of a shape tends to increase much slower, meaning that even relatively large paths can be represented in an efficient way.
