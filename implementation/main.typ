...
Once this is done, control is passed back to the user, in which case they have two choices: Either, they keep rendering more paths, in which case the above-mentioned stages are re-run for the new paths, or the user calls the `render_to_pixmap` method, in which case _fine rasterization_ kicks off.
...
The information that is stored for each tile is shown in @tile-fields. One the one hand, we store the x and y coordinates (in tile coordinates instead of pixel coordinates), but on the other hand, we also keep track of the line associated with the tile. We also store a special boolean flag `has_winding` which will be activated for any tile where the line intersects the top part, as indicated by the yellow points in @generating-tiles. This information will be needed later.
...
The only difference between fill and alpha fill commands lies in the fact that alpha fills have an additional `alpha_idx` property. Normal fill commands are used for filling the areas _in-between_ strips where there is no anti-aliasing, while alpha fill commands are used for the regions inside of strips which require an additional opacity factor to be applied to the pixels.
...
Given a set of wide tiles that contain drawing commands, the actual rasterization process will now be started. As part of this stage, we calculate the RGBA values of each pixel by iterating over each wide tile and processing them in isolation. Every time we go to a new wide tile, an empty scratch buffer with the dimension 256x4 pixels (i.e. the same dimension as a wide tile) is created and each pixel is set to the RGBA value (0.0, 0.0, 0.0, 0.0), so a fully transparent color. Note that pixel values are stored in _column-major_ order. The rationale for doing so is that it improves cache efficiency, since the pixels that are processed during each command will be stored contiguously in memory.
...
Two key implementation details make sure that this property is always upheld. First, before dispatching a path rendering task, the main thread assigns a unique ID to the task that is based on an incrementing counter. By doing so, just checking the ID of a render task is enough to determine if it should be processed next during coarse rasterization or whether there are in-between tasks still missing. Secondly, in order to send the rendered strips back to the main thread, we use the ordered-channel#footnote[https://gitlab.com/kornelski/ordered-channel (accessed on 11.09.2025)] crate, which provides a MPSC (multi-producer single-consumer) channel primitive with the useful property that the user will always receive messages with incrementally ascending IDs. Internally, this is achieved with the help of a binary heap and a simple counter keeping track of the next expected ID in the `Receiver`. In case the receiver gets a message with the expected ID, it simply forwards it to the user. If the ID of the received message is too high, it is instead pushed to the binary heap and stored there until it is the right time to forward it. This process is repeated until the message with the right ID is received, which will then again be forwarded to the user. Using this principle, the user will always receive the messages in the ascending order, regardless of the order in which the messages where sent from each thread.
...
As was previously shown in @api_example_Listing, once the user is done submitting commands, they need to call the `flush` method, which starts the second coarse rasterization phase. Similarly to the first phase, the main thread will try to receive new strips from the queue and do coarse rasterization for them. However, unlike previously, the main thread will not pass control back to the user in case no strips are currently available. Instead, it will _block_ by continiously polling for new sparse strips in a loop, until the very last batch of strips has been received and processed in coarse rasterization. It is important that we do this, because as soon as the rasterization stage is started, it is assumed that all render commands are already stored in their wide tiles.
