...
== Packing <packing>
After completing fine rasterization, each wide tile stores a buffer of RGBA values for each pixel that is covered by the wide tile. However, there are three problems that need to be addressed:
- The wide tiles (and in particular their buffers) are not stored contiguously in memory. However, users usually expect a continuous buffer of bytes for the _whole_ area.
- The wide tiles themselves store there pixel in column-major order, while the user expects pixels in row-major order.
- In case we are running the high-precision rasterization pipeline, the RGBA values will be stored as normalized `f32` values, but the user usually expects `u8` values for the final pixel values, meaning that we must convert them.
...
#todo[mention transpose, add a graphic]
...
The core insight necessary to understand how multi-threading works in the path rendering stage is that path rendering (ranging from stroke expansion or flattening to strips generation) is essentially a pure function that takes a single path as input and returns a vector of strips as the output. Since there are no other dependencies, the rendering of a single path can be completely outsourced without requiring any additional communication between the main thread and the child thread until the strips have been generated. Since these calculations often take up a huge chunk of the time, being able to render multiple threads in parallel can lead to very considerable speedups.

In practice, how this works is that when the user emits a command like `fill_path` or `stroke_path`, the main thread first stores the command inside of a queue. The reason for this is that we make use of a _batching mechanism_ to increase the efficiency of the whole process. While multi-threading itself is powerful, a problem is that if the amount of work we farm out to the threads is very small, the overhead that arises from context switches and the bookkeeping done by rayon will be so large that any benefit of parallelism is completely eclipsed. 

Therefore, the main thread can collect multiple drawing commands and keep track of a counter that tries to _estimate_ the cost of rendering all batched paths. Only once a certain threshold is reached does the main thread dispatch the rendering tasks to a thread in the thread pool.

The cost estimation function works in a somewhat naive way by just considering important information about the path (the number of line segments and curve segments, the overall length of the path and whether we are performing filling or stroking), but has empirically been validated to mostly work well. Using this method, if the user for example decides to draw many small rectangles, the main thread will collect dozens of these commands before dispatching them, while if larger geometries with many curves are drawn, only very few of them will be batched. To enable the communication between the main thread and the worker threads, we use the `crossbeam-channel` crate @crossbeam-channel that provides primitives for implementing the single-producer multi-consumer paradigm.
