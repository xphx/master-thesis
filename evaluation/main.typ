...
Let us shift our focus to @solid-fill-fish next, where we can see one of the areas where Vello CPU performs slightly worse. While performance is yet again only second to Blend2D for larger sizes, for 8x8 and sometimes 16x16 we are a bit slower than some other renderers like JUCE. However, we believe that this discrepancy could be easily fixed by implementing an optimization that has not been implemented so far: A look at the profiler reveals that more than 60% of the time is spent in the curve flattening stage. Currently, each curve goes through the whole flattening process regardless of its size. But for such a small geometry, it is safe to assume that many of the curves are so short that they could just be straight-up approximated by a single line instead of tediously going through all steps of flattening. We believe that once a heuristic is implemented that handles such curves more efficiently, it should be possible close that gap.
...
As can be seen in @mt-solid-fill-polynz40, things improve as the complexity of the path increases. In the given figure, we only consider line-based shapes meaning that the most time-consuming step is strip generation, but as can be seen we already achieve much higher speedups in this test case. For small shapes, we are still restricted to a speedup of less than two (note in particular how the speedup actually gets worse as the thread number increases), but for larger versions of the shape, the speedup gets increasingly better.

Once curve segments and strokes are thrown into the mix, the path processing time increases even further, leading to even higher speedups as can be observed in @mt-solid-fill-fish. Blend2D achieves a speedup of around 3x-4x at most, while Vello CPU is able to push the boundary further to a speedup of up to 5x.
...
Finally, it is important to remember that the second component that can be parallelized in the pipeline concerns fine rasterization, which can become a bottleneck when drawing shapes with complex paints. As can be seen in @mt-linear-fill-recta, we do get higher speedups when filling rectangles using a linear gradient instead of solid colors (for reasons that are not clear, Blend2D actually experiences a slowdown for small sizes), although the speedup is still far from ideal. To us, this is unexpected as profiling reveals that more than 90% of the time is indeed spent in fine rasterization, which is more trivial to parallelize as there are no dependencies between the wide tiles. We would therefore expect test cases where fine rasterization is the bottleneck to yield higher relative speedups than test cases where path rendering consumes the most time, which does not seem to be the case.
...
We conclude that both renderers have powerful multi-threading capabilities but with different strengths and weaknesses. Blend2D seems to achieve better thread utilization when rendering small or simple paths, while Vello CPU seems to have a slight edge when rendering paths with many curves or strokes. Profiling reveals that it is likely that Vello CPU could achieve even higher speedups if the coarse rasterization bottleneck is eliminated, although that remains to be empirically validated as part of the future work.
