...
Let us shift our focus to @solid-fill-fish next, which forms the main exception to our previous statement that Vello CPU has a weakness for small shapes and actually ends up beating Blend2D for certain sizes. The reason that Vello CPU performs so well here is to a large degree due to the small-curve optimization that was introduced in @flatten_opt. As can be seen in the figure, having this shortcut path leads to impressive gains if the shape consists of many small curve segments. Blend2D does regain the lead as the shape sizes increases, but Vello CPU is still significantly faster than all other implementations.
