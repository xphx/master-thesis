...
Even when doing 2D rendering on the CPU instead of the GPU, having good performance is still very crucial. In this section, we will analyze how Vello CPU fares in comparison to other prominent 2D renderers by evaluating its performance using the Blend2D benchmark suite. 

== Introduction
The main idea behind the benchmark harness is to the renderer feed various inputs that exercise different parts of the rendering pipeline to varying degrees, measure the time and compare the results. Doing so allows us to easily determine both the strengths and weaknesses of each renderer. To achieve this, the harness defines a number of different parameters that can be tweaked to specify what exactly should be rendered. The configuration possibilities are visualized in @bench-types.
...
- *Rect Round*: Rectangles with rounded corners. The reason these are worth benchmarking specifically is that rounded rectangles are very common in graphical user interfaces and should therefore be fast to render.
- *Triangle*: Triangles. They represent the most basic form of polygon.
...
Overall, we draw the conclusion that there definitely is more work left to be done on the stroking side of things, but the performance is at least comparable to other renderer and does stand out when considering larger shape sizes.
