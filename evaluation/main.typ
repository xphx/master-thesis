...
Since Vello CPU is a Rust-based renderer, we had to create C bindings#footnote[https://github.com/LaurenzV/vello_cpu_c] to properly integrate it into the harness. Given that we used Rust as the implementation language, another intriguing research question for us was how the renderer compares against other Rust-based CPU renderers. To this end, we also created C bindings#footnote[https://github.com/LaurenzV/raqote_c]#footnote[https://github.com/LaurenzV/tiny_skia_c] for `tiny-skia`#footnote[https://github.com/linebender/tiny-skia] and `raqote`#footnote[https://github.com/jrmuizel/raqote], the two currently most commonly used renderers in the Rust ecosystem. To make the comparison fair, all crates have been compiled with the flag `target-feature=+avx2` so that the compiler can make use of AVX2 instructions. On ARM, no additional flags were necessary since the compiler can assume that `NEON` intrinsics are available. To avoid outliers, each benchmark was repeated 10 times and the best result was used from those runs.