...
In @solid-stroke-butterfly, we can see the running times when stroking a curved shape instead. We once again observe that in this particular case, Vello CPU does not perform as well as other renderers for small shape sizes, but makes up for the differences as the dimensions of the shape increases. In this case, we do not attribute the sluggish performance for small shape sizes to the fixed size of 4x4 for a tile. A look at the profile in @solid-stroke-butterfly-profile reveals that more than 70% of the time is spent in stroke expansion, leading us to the conclusion that there seems to be some inefficiency in the current algorithm. The exact reason for this issue remains to be determined, however.
