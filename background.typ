...
As mentioned above, a set of very basic drawing primitives is required to be able to define the outlines of graphical objects. There is no unanimously recognized set of such building blocks, and different specifications have different requirements in this regard. For example, the PDF (portable document format) specification only defines lines and cubic Bézier curves as the basic path-building primitives #cite(<pdf_spec>, supplement: [p. 132-133]), while the SVG (scalable vector graphics) specification additionally also allows using quadratic Bézier curves and elliptic arc curves #cite(<svg1_spec>, supplement: [ch. 8]).
...
Each type of primitive has a start point $P_0$ and an end point $P_1$ defined in the 2D coordinate system. We can then define a parametric variable $t in [0.0, 1.0]$ as well as a parametric function $F$ such that $F(0) = P_0$, $F(1) = P_1$, and $F(t) = P_i$, where $P_i$ simply represents the position of the interpolated point for the given drawing primitive. By then conceptually evaluating the function _infinitely_ many times for all values in the interval $[0, 1]$, we can plot its exact representation.

=== Lines
The definition of lines is relatively straight-forward and illustrated in @line_definition. Given our start and end points $P_0$ and $P_1$, we can use the formula $F(t) = P_0 + t (P_1 - P_0)$ (a simple linear interpolation between the two points) to evaluate it. When doing so for all $t in [0,1]$, we end up with a straight line that connects the two points.
